...
##visualization 
dfRslt.avg <- 
        ddply(V2data, .(province), summarise,
              avg.caseRatio      = mean(caseRatio),
              avg.caseRatio0     = mean(caseRatio0),
              avg.MovGP          = mean(grocery_and_pharmacy),
              avg.MovRR          = mean(retail_and_recreation),
              avg.MovP           = mean(parks),
              avg.MovW           = mean(workplaces))

dfRslt.sub <- merge(V2data, dfRslt.avg, by="province")


dfRslt.sub$diff.caseRatio   <- dfRslt.sub$caseRatio             - dfRslt.sub$avg.caseRatio
dfRslt.sub$diff.caseRatio0  <- dfRslt.sub$caseRatio0            - dfRslt.sub$avg.caseRatio0
dfRslt.sub$diff.MovGP       <- dfRslt.sub$grocery_and_pharmacy  - dfRslt.sub$avg.MovGP
dfRslt.sub$diff.MovRR       <- dfRslt.sub$retail_and_recreation - dfRslt.sub$avg.MovRR 
dfRslt.sub$diff.MovP        <- dfRslt.sub$parks                 - dfRslt.sub$avg.MovP
dfRslt.sub$diff.MovW        <- dfRslt.sub$workplaces            - dfRslt.sub$avg.MovW



ggplot(dfRslt.sub, aes(x = diff.MovGP, y = diff.caseRatio)) +
        geom_point(aes(color = province), size = 2) +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        xlab("Visits to grocery and pharmacy locations") +
...
ggsave(paste0(dirRslt, "MovGP_to_caseRatio.png"))

ggplot(dfRslt.sub, aes(x = diff.MovW , y = diff.caseRatio)) +
        geom_point(aes(color = province), size = 2) +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        xlab("Visits to workplaces") +
        ylab("Case ratio with 14 days lag") + 
        theme(axis.title = element_text(size = rel(2.5)),
              axis.text = element_text(size = rel(2)),
              legend.text = element_text(size=20),
              legend.title = element_text(size=20)) +
        guides(color = guide_legend(override.aes = list(size = 5)))
ggsave(paste0(dirRslt, "MovW_to_caseRatio.png"))


ggplot(dfRslt.sub, aes(x = diff.caseRatio0 , y = diff.MovRR)) +
        geom_point(aes(color = province), size = 2) +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        xlab("Present case ratio") +
        ylab("Visits to retail and recreation locations") + 
        theme(axis.title = element_text(size = rel(2.5)),
              axis.text = element_text(size = rel(2)),
              legend.text = element_text(size=20),
              legend.title = element_text(size=20)) +
        guides(color = guide_legend(override.aes = list(size = 5)))
ggsave(paste0(dirRslt, "caseRatio0_to_MovRR.png"))


ggplot(dfRslt.sub, aes(x = diff.caseRatio0 , y = diff.MovGP)) +
        geom_point(aes(color = province), size = 2) +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        xlab("Present case ratio") +
        ylab("Visits to grocery and pharmacy locations") + 
        theme(axis.title = element_text(size = rel(2.5)),
              axis.text = element_text(size = rel(2)),
              legend.text = element_text(size=20),
              legend.title = element_text(size=20)) +
        guides(color = guide_legend(override.aes = list(size = 5)))
ggsave(paste0(dirRslt, "caseRatio0_to_MovGP.png"))


ggplot(dfRslt.sub, aes(x = diff.caseRatio0 , y = diff.MovP)) +
        geom_point(aes(color = province), size = 2) +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        xlab("Present case ratio") +
        ylab("Visits to parks") + 
        theme(axis.title = element_text(size = rel(2.5)),
              axis.text = element_text(size = rel(2)),
              legend.text = element_text(size=20),
              legend.title = element_text(size=20)) +
        guides(color = guide_legend(override.aes = list(size = 5)))
ggsave(paste0(dirRslt, "caseRatio0_to_MovP.png"))
